...
function render(app){
  ...
  const viewMatrix = matrix4.perspective(state.fovy, gl.canvas.width / gl.canvas.height, 0.1, gl.canvas.width)    //perspective: (fovy, aspect, near, far)
    ;
...
function createModelBufferArrays() {
  ...
  const a_position = [
    ...
    ...rectVertices(points[7], points[6], points[5], points[4]), //後
    ...
    ...rectVertices(points[6], points[2], points[1], points[5]), //下
  ];
